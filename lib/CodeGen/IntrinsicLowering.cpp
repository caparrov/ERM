...
static CallInst *ReplaceCallWith(const char *NewFn, CallInst *CI,
  ...
  if (!CI->use_empty()){
    //  dbgs() << "Replacing all uses with a value of type "<< NewCI->getType()->getTypeID()<<"\n";
...
void IntrinsicLowering::LowerIntrinsicCall(CallInst *CI) {
  ...
  switch (Callee->getIntrinsicID()) {
    ...
    default:{
      ...
      //   assert(0==0 && "Replacing call\n");
      
      
      /*  START COMMENT OF WORKING
       errs() << "Iterating through the uses of the old instruction....\n";
       for(Value::use_iterator i = CI->use_begin(), ie = CI->use_end(); i!=ie; ++i){
       errs() << "Use of the CI node " << *i << "\n";
       }
       
       
       Value *V = ReplaceCallWith("llvm.x86.sse3.hadd.ps", CI, CS.arg_begin(), CS.arg_end(),
       CI->getArgOperand(1)->getType());
       
       dbgs() << "Generated value type " << V->getType()->getTypeID() << "\n";
       dbgs() << "Operand type " << Twine(CI->getArgOperand(1)->getType()->getTypeID()) << "\n";
       
       
       END COMMENT OF WORKING */
      
      // if (!CI->getType()->isVoidTy())
      //  CI->replaceAllUsesWith(V);
      //// assert(0==1 && "All uses set\n");
      //  report_fatal_error("Code generator does not support intrinsic function '"+
      //     Callee->getName()+"'!");
      
      report_fatal_error("Code generator does not support intrinsic function '"+Callee->getName()+"'!");
      break;
    }
    ...
    case Intrinsic::objectsize:
      //      unsigned int size = cast<IntegerType>(ki->inst->getType())->getBitWidth();
      CI->replaceAllUsesWith(ConstantInt::getSigned(IntegerType::get(Context, 64),-1));
      break;
