...
DynamicAnalysis::DynamicAnalysis (string TargetFunction,
							...
							unsigned RegisterFileSize,
                                  ...
                                  bool ConstraintAGUs,
                                  int rep, bool InOrderExecution, bool ReportOnlyPerformance, unsigned PrefetchLevel,
                                  unsigned PrefetchDispatch, unsigned PrefetchTarget)
{
                                                ...
                                                unsigned NElementsVector, int IssuePort, bool isPrefetch)
{
  ...
  /*
   ...
   ExecutionUnitsThroughput[ExecutionResource] * ExecutionUnitsParallelIssue[ExecutionResource]) {
   Node->widthOccupancy += AccessWidth * NElementsVector;
   }
   else {
   // If ExecutionUnitsThroughput[ExecutionResource] < 1, AccessWidth*NElementsVector will never be less than one,
  ...
  /*
   ...
   DEBUG (dbgs () <<
   "Level got full because NodeIssueOccupancy == (unsigned)ExecutionUnitsParallelIssue[ExecutionResource]  \n");
   #endif
   }
   }
   else {
   ...
   DEBUG (dbgs () <<
   "Level got full becauseNodeWidthOccupancy == (unsigned)ExecutionUnitsParallelIssue[ExecutionResource]*ExecutionUnitsThroughput[ExecutionResource]  \n");
   #endif
   }
   if (NodeIssueOccupancy == (unsigned) ExecutionUnitsParallelIssue[ExecutionResource]) {
   LevelGotFull = true;
    ...
    for (uint64_t i = First + 1; i <= LastCycle; i++) {
      ...
      for (int j = 0; j < NResources; j++) {
        ...
        if (i <= LastIssueCycleVector[ResourceType]
            /*GetLastIssueCycle(ResourceType, WithPrefetch) */
            ) {
          IsInAvailableCyclesTree = false;
          IsInFullOccupancyCyclesTree = false;
  ...
  for (int j = 0; j < NResources; j++) {
    ...
    if (InstructionsCountExtended[ResourceType] > 0) {
      ...
      if (EmptyLevel == true) {	// This will be only executed the first time of a non-empty level
        EmptyLevel = false;
        First = FirstNonEmptyLevel[ResourceType];
        //    MaxLatency = ceil(AccessWidth/ExecutionUnit[sThroughput[ResourceType]);
  ...
  if (EmptyLevel == false) {
    ...
    for (unsigned i = First + 1; i <= LastCycle; i++) {
      ...
      for (int j = 0; j < NResources; j++) {
        ...
        if (i <= LastIssueCycleVector[ResourceType]
            /*GetLastIssueCycle(ResourceType, 0) */
            ) {
          IsInAvailableCyclesTree = false;
          IsInFullOccupancyCyclesTree = false;
...
#ifdef DEBUG_OOO_BUFFERS
  // dbgs() << "Size after removing "<< node_size(LoadBufferCompletionCyclesTree) <<"\n";
  ...
  if(condition==true){
    ...
    for(std::vector<uint64_t>::iterator it = n->IssueCycles.begin(); (!StopChecking && it != n->IssueCycles.end()); ++it) {
      ...
      if (*it <= i && *it != 0) {
        ...
        if(!StopChecking){
          //  dbgs () << "Decreasing iterator\n";
          //  it--;
          // dbgs () << "Decreased it\n";
        }else{
          //n = DispatchToLoadBufferQueueTree;
          // dbgs () << "DId not Decreased it\n";
        }
      }
    }
...
{
  /*
   dbgs () << "Dispatch to Load Buffer Issue Cycles:\n";
   PrintDispatchToLoadBufferTreeRecursive (DispatchToLoadBufferQueueTree, false);
   dbgs () << "\n";
   dbgs () << "Dispatch to Load Buffer Completion Cycles:\n";
  ...
  for (size_t j = 0; j < ResourcesVector.size (); j++) {
    ...
    if (CGSFCache[ResourcesVector[j]].size () != 0 && CGSFCache[ResourcesVector[j]].count () != 0) {
      ...
      if (BitMesh.count() == 0) {
        ...
        /*  if (ResourcesVector[j] >= RS_STALL && ResourcesVector[j] <= LFB_STALL) {
         BitMesh |= CGSFCache[ResourcesVector[j]];
...
DynamicAnalysis::finishAnalysisContechSimplified ()
{
  ...
  // ===================== ALL OVERLAPS ===========================//
  // This approach calculates overlap cycles of every combination of resources,
  // including always all the buffers. The probablity that all of them
  // overlap is very low, that is why overlap is almost always zero.
  
  /*
   {
   
   printHeaderStat ("All overlaps");
   unsigned n = nExecutionUnits;
   int nCombinations = 0;
   for (unsigned k = 2; k <= nExecutionUnits; k++) {
