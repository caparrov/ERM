                                  ...
                                  unsigned PrefetchDispatch, unsigned PrefetchTarget)
{
  ...
  if(L1CacheSize != 0 &&  ExecutionUnitsLatency[L2_LOAD_NODE] <= ExecutionUnitsLatency[L1_LOAD_CHANNEL])
    report_fatal_error("Latency of L1 cache cannot be larger or equal than latency to other levels");
  if(L2CacheSize != 0 &&  ExecutionUnitsLatency[L3_LOAD_NODE] <= ExecutionUnitsLatency[L2_LOAD_CHANNEL])
    report_fatal_error("Latency of L2 cache cannot be larger or equal than latency to other levels");
  if(LLCCacheSize != 0 &&  ExecutionUnitsLatency[MEM_LOAD_NODE] <= ExecutionUnitsLatency[L3_LOAD_CHANNEL])
