...
static cl::opt<string> TargetFunction("function",
		cl::desc("Name of the function to be analyzed. Default is main."),
		cl::init("main"));

static cl::opt<unsigned> MemoryWordSize("memory-word-size",
		cl::desc(
				"Size in bytes of a memory word. Default value is 8 (floating-point double precision)"),
		cl::init(8));

static cl::opt<unsigned> CacheLineSize("cache-line-size",
		cl::desc("Cache line size (B). Default value is 64 B"), cl::init(8));

static cl::opt<unsigned> RegisterFileSize("register-file-size",
		cl::desc("Size of the register file. Default value is 0"), cl::init(0));

static cl::opt<unsigned> L1CacheSize("l1-cache-size",
		cl::desc("Size of the L1 cache (in bytes). Default value is 0"),
		cl::init(0));

static cl::opt<unsigned> L2CacheSize("l2-cache-size",
		cl::desc("Size of the L2 cache (in bytes). Default value is 256 KB"),
		cl::init(0));

static cl::opt<unsigned> LLCCacheSize("llc-cache-size",
		cl::desc("Size of the L3 cache (in bytes). Default value is 20 MB"),
		cl::init(0));

static cl::opt<string> Microarchitecture("uarch",
		cl::desc("Name of the microarchitecture"), cl::init("SB"));

static cl::list<float> ExecutionUnitsLatency("execution-units-latency",
		cl::CommaSeparated,
		cl::desc(
...
static cl::list<double> ExecutionUnitsThroughput("execution-units-throughput",
		cl::CommaSeparated,
		cl::desc(
...
static cl::list<int> ExecutionUnitsParallelIssue(
		"execution-units-parallel-issue", cl::CommaSeparated,
		cl::desc(
...
static cl::list<unsigned> MemAccessGranularity("mem-access-granularity",
		cl::CommaSeparated,
		cl::desc(
...
static cl::opt<unsigned> AddressGenerationUnits("address-generation-units",
		cl::desc(
				"Number of address generation units. Default value is infinity"),
		cl::init(-1));

static cl::opt<int> IFB("instruction-fetch-bandwidth",
		cl::desc("Size of the reorder buffer. Default value is infinity"),
		cl::init(-1));

static cl::opt<unsigned> ReservationStation("reservation-station-size",
		cl::desc(
				"Size of a centralized reservation station. Default value is infinity"),
		cl::init(0));

static cl::opt<unsigned> ReorderBuffer("reorder-buffer-size",
		cl::desc("Size of the reorder buffer. Default value is infinity"),
		cl::init(0));

static cl::opt<unsigned> LoadBuffer("load-buffer-size",
		cl::desc("Size of the load buffer. Default value is infinity"),
		cl::init(0));

static cl::opt<unsigned> StoreBuffer("store-buffer-size",
		cl::desc("Size of the store buffer. Default value is infinity"),
		cl::init(0));

static cl::opt<unsigned> LineFillBuffer("line-fill-buffer-size",
		cl::desc(
				"Specify the size of the fill line buffer. Default value is infinity"),
		cl::init(0));

static cl::opt<bool> WarmCache("warm-cache",
		cl::desc(
				"Enable analysis of application in a warm cache scenario. Default value is FALSE"),
		cl::init(false));

static cl::opt<bool> x86MemoryModel("x86-memory-model",
		cl::desc("Implement x86 memory model. Default value is FALSE"),
		cl::init(false));

static cl::opt<bool> ConstraintPorts("constraint-ports",
		cl::desc(
				"Block the ports while the instruction is being issued according to the corresponding throughput.\nDefault value is FALSE"),
		cl::init(false));

static cl::opt<bool> ConstraintAGUs("constraint-agus",
		cl::desc(
				"Constraint agus according to specified architecture. Default value is FALSE"),
		cl::init(false));

static cl::opt<bool> ConstraintPortsx86("constraint-ports-x86",
		cl::desc(
				"Constraint ports dispatch according to x86 architecture. Default value is FALSE"),
		cl::init(false));

static cl::opt<bool> SpatialPrefetcher("spatial-prefetcher",
		cl::desc("Implement spatial Prefetching. Default value is FALSE"),
		cl::init(false));

static cl::opt<unsigned> PrefetchLevel("prefetch-level",
		cl::desc(
...
static cl::opt<unsigned> PrefetchDispatch("prefetch-dispatch",
		cl::desc(
...
static cl::opt<unsigned> PrefetchTarget("prefetch-target",
		cl::desc(
...
static cl::opt<bool> InOrderExecution("in-order-execution",
		cl::desc("In order execution. Default value is FALSE"),
		cl::init(false));

static cl::opt<bool> ReportOnlyPerformance("report-only-performance",
		cl::desc("Reports only performance (op count and span)"),
		cl::init(false));
...
static GenericValue executeFCMP_ORD(GenericValue Src1, GenericValue Src2,
	...
	if (Ty->isVectorTy()) {
		...
		Dest.IntVal = APInt(1,
				(Src1.FloatVal == Src1.FloatVal
						&& Src2.FloatVal == Src2.FloatVal));
	else {
		Dest.IntVal = APInt(1,
				(Src1.DoubleVal == Src1.DoubleVal
    ...
    if (dyn_cast<VectorType>(Ty)->getElementType()->isDoubleTy())     \
      ...
      llvm_unreachable(0);                                            \
		...
		case Instruction::FRem:
			if (dyn_cast<VectorType>(Ty)->getElementType()->isFloatTy())
				for (unsigned i = 0; i < R.AggregateVal.size(); ++i)
					R.AggregateVal[i].FloatVal = fmod(
							Src1.AggregateVal[i].FloatVal,
							Src2.AggregateVal[i].FloatVal);
			else {
				if (dyn_cast<VectorType>(Ty)->getElementType()->isDoubleTy())
					for (unsigned i = 0; i < R.AggregateVal.size(); ++i)
						R.AggregateVal[i].DoubleVal = fmod(
								Src1.AggregateVal[i].DoubleVal,
...
GenericValue * Interpreter::visitAllocaInst(AllocaInst &I) {
	...
	DEBUG(
			dbgs() << "Allocated Type: " << *Ty << " (" << TypeSize
...
GenericValue * Interpreter::visitCallSite(CallSite CS) {
	...
	if (F && F->isDeclaration())
		...
		case Intrinsic::vacopy:   // va_copy: dest = src
			SetValue(CS.getInstruction(), getOperandValue(*CS.arg_begin(), SF),
					SF);
			return NULL;
		default:
...
//VCA: comment
	...
	uint16_t pNum = 1;
	for (CallSite::arg_iterator i = SF.Caller.arg_begin(), e =
			SF.Caller.arg_end(); i != e; ++i, ++pNum) {
		Value *V = *i;
		ArgVals.push_back(getOperandValue(V, SF));
...
GenericValue * Interpreter::visitShl(BinaryOperator &I) {
	...
	if (Ty->isVectorTy()) {
		...
		for (unsigned i = 0; i < src1Size; i++) {
			...
			Result.IntVal = valueToShift.shl(
					getShiftAmount(shiftAmount, valueToShift));
...
GenericValue * Interpreter::visitLShr(BinaryOperator &I) {
	...
	if (Ty->isVectorTy()) {
		...
		for (unsigned i = 0; i < src1Size; i++) {
			...
			uint64_t shiftAmount = Src2.AggregateVal[i].IntVal.getZExtValue();
			llvm::APInt valueToShift = Src1.AggregateVal[i].IntVal;
			Result.IntVal = valueToShift.lshr(
					getShiftAmount(shiftAmount, valueToShift));
			Dest.AggregateVal.push_back(Result);
		}
	} else {
...
GenericValue * Interpreter::visitAShr(BinaryOperator &I) {
	...
	if (Ty->isVectorTy()) {
		...
		for (unsigned i = 0; i < src1Size; i++) {
			...
			uint64_t shiftAmount = Src2.AggregateVal[i].IntVal.getZExtValue();
			llvm::APInt valueToShift = Src1.AggregateVal[i].IntVal;
			Result.IntVal = valueToShift.ashr(
					getShiftAmount(shiftAmount, valueToShift));
			Dest.AggregateVal.push_back(Result);
		}
	} else {
...
GenericValue Interpreter::executeTruncInst(Value *SrcVal, Type *DstTy,
	...
	if (SrcTy->isVectorTy()) {
		...
		for (unsigned i = 0; i < NumElts; i++)
			Dest.AggregateVal[i].IntVal = Src.AggregateVal[i].IntVal.trunc(
					DBitWidth);
	} else {
...
GenericValue Interpreter::executeSExtInst(Value *SrcVal, Type *DstTy,
	...
	if (SrcTy->isVectorTy()) {
		...
		for (unsigned i = 0; i < size; i++)
			Dest.AggregateVal[i].IntVal = Src.AggregateVal[i].IntVal.sext(
					DBitWidth);
	} else {
...
GenericValue Interpreter::executeZExtInst(Value *SrcVal, Type *DstTy,
	...
	if (SrcTy->isVectorTy()) {
		...
		Dest.AggregateVal.resize(size);
		for (unsigned i = 0; i < size; i++)
			Dest.AggregateVal[i].IntVal = Src.AggregateVal[i].IntVal.zext(
					DBitWidth);
	} else {
		const IntegerType *DITy = cast<IntegerType>(DstTy);
		unsigned DBitWidth = DITy->getBitWidth();
...
GenericValue Interpreter::executeFPTruncInst(Value *SrcVal, Type *DstTy,
	...
	if (SrcVal->getType()->getTypeID() == Type::VectorTyID) {
		assert(
				SrcVal->getType()->getScalarType()->isDoubleTy()
		...
		assert(
				SrcVal->getType()->isDoubleTy() && DstTy->isFloatTy()
						&& "Invalid FPTrunc instruction");
...
GenericValue Interpreter::executeFPExtInst(Value *SrcVal, Type *DstTy,
	...
	if (SrcVal->getType()->getTypeID() == Type::VectorTyID) {
		assert(
				SrcVal->getType()->getScalarType()->isFloatTy()
						&& DstTy->getScalarType()->isDoubleTy()
		...
		assert(
				SrcVal->getType()->isFloatTy() && DstTy->isDoubleTy()
						&& "Invalid FPExt instruction");
...
GenericValue Interpreter::executeFPToUIInst(Value *SrcVal, Type *DstTy,
	...
	if (SrcTy->getTypeID() == Type::VectorTyID) {
		...
		if (SrcVecTy->getTypeID() == Type::FloatTyID) {
			assert(
					SrcVecTy->isFloatingPointTy()
							&& "Invalid FPToUI instruction");
			for (unsigned i = 0; i < size; i++)
		...
		if (SrcTy->getTypeID() == Type::FloatTyID)
			...
			Dest.IntVal = APIntOps::RoundDoubleToAPInt(Src.DoubleVal,
					DBitWidth);
		}
	}
...
GenericValue Interpreter::executeSIToFPInst(Value *SrcVal, Type *DstTy,
	...
	if (SrcVal->getType()->getTypeID() == Type::VectorTyID) {
		...
		if (DstVecTy->getTypeID() == Type::FloatTyID) {
			...
			for (unsigned i = 0; i < size; i++)
				Dest.AggregateVal[i].DoubleVal =
						APIntOps::RoundSignedAPIntToDouble(
								Src.AggregateVal[i].IntVal);
		}
	} else {
		// scalar
...
GenericValue * Interpreter::visitExtractValueInst(ExtractValueInst &I) {
	...
	Type *IndexedType = ExtractValueInst::getIndexedType(Agg->getType(),
			I.getIndices());
...
void Interpreter::callFunction(Function *F,
	...
	assert(
			(ECStack.empty() || ECStack.back().Caller.getInstruction() == 0
...
void Interpreter::run() {
	...
	clock_t tStart, tEnd, tStartPostProcessing, tEndPostProcessing,
			tStartCacheWarmed, tEndCacheWarmed;
	float Cycles, ExecutionTime, CyclesPostProcessing,
			ExecutionTimePostProcessing, ExecutionTimeActualSimulation;

	Analyzer = new DynamicAnalysis(TargetFunction, Microarchitecture,
			MemoryWordSize, CacheLineSize, RegisterFileSize, L1CacheSize,
			L2CacheSize, LLCCacheSize, ExecutionUnitsLatency,
			ExecutionUnitsThroughput, ExecutionUnitsParallelIssue,
			MemAccessGranularity, AddressGenerationUnits, IFB,
			ReservationStation, ReorderBuffer, LoadBuffer, StoreBuffer,
			LineFillBuffer, WarmCache, x86MemoryModel, SpatialPrefetcher,
			ConstraintPorts, ConstraintPortsx86, ConstraintAGUs, 0,
			InOrderExecution, ReportOnlyPerformance, PrefetchLevel,
	...
	if (WarmCache && TargetFunction.compare("main") == 0) {
		report_fatal_error(
				"Warm cache scenario for main function requires two executions.\n");
	...
	while (!ECStack.empty()) {
		...
		bool isTargetFunction = (I.getParent()->getParent()->getName().find(
				TargetFunction) != string::npos);
		bool isDebugInstruction = (I.getOpcode() == Instruction::Call
				&& (I.getOperand(I.getNumOperands() - 1))->getName().str().find(
						"llvm.dbg") != string::npos);
		...
		if (isTargetFunction == true && Analyzer->rep == 1 && !WarmCache) {
			report_fatal_error(
					"The target function was called twice in a cold cache scenario\n");
...
//CyclesActualSimulation=(tEndCacheWarmed - tStartCacheWarmed);
						...
						ExecutionTimeActualSimulation =
								((double) (tEndCacheWarmed - tStartCacheWarmed))
										/ CLOCKS_PER_SEC;
