...
static bool ffiInvoke(RawFunc Fn, Function *F,
  ...
  //New VCA
  std::vector<ffi_type*> args(NumArgs);
  unsigned int NArgs = (int)ArgTypes.size();
  
  unsigned ArgBytes = 0;
  
  
  // ArgVals is a vector of GenericValue. How to get the type of a GenericValue
  // Now we have to change the condition!!

  if (NumArgs > F->arg_size() && F->isVarArg()) {
    for (unsigned int i = 0; i < NArgs; i++) {
      const unsigned ArgNo = i;
